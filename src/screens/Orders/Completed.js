...
export default class Received extends React.Component {
    ...
    back(oid) {
        firebase.database().ref('orders/'+oid).update({ status: 2 })
    }
    ...
    orderCard(oid, order) {
        ...
        <View style={{ width:'100%', flexDirection:'row', marginTop: '2%', marginBottom: '2%' }} >
            ...
            <View style={{width:'96%', height:'100%', justifyContent: 'flex-start', alignItems: 'center', marginLeft: '2%' }}>
    ...
    renderItems()
    {
        ...
        for(let i=0; i<oids.length; i++)
            if(this.state.orders[oids[i]].status == 3)
